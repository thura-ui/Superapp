...
import { ChevronLeft, ChevronRight, CheckCircle2, AlertCircle, HelpCircle, Smartphone } from 'lucide-react';
...
export default function EsimInstallationGuide({ onBack }: EsimInstallationGuideProps) {
  ...
  // 🌟 Troubleshooting Data များကို Language ပြောင်းတိုင်း update ဖြစ်စေရန် Component အတွင်းသို့ ရွှေ့ပြောင်းထားပါသည်
  ...
  return (
    <div className="w-full min-h-screen bg-slate-50/50 py-4 sm:py-8 selection:bg-blue-500/10 font-['Poppins'] text-slate-900">
      <div className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-4 space-y-4 sm:space-y-6">
        
        {/* Top Title Header */}
        <div className="bg-white/80 backdrop-blur-xl border border-slate-200/60 p-4 sm:p-5 rounded-2xl shadow-sm flex items-center justify-start">
          <h1 className="text-base sm:text-lg font-semibold text-slate-900 tracking-tight text-left font-['Poppins']">
            {t('travelEsimUserGuide')}
          </h1>
        </div>

        {/* Important Notice Banner */}
        <div className="bg-gradient-to-r from-blue-500/10 via-cyan-500/10 to-teal-500/10 rounded-2xl p-4 sm:p-5 border border-blue-200/50 flex items-start gap-3.5 shadow-sm">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-blue-600 flex items-center justify-center shrink-0 shadow-md shadow-blue-500/20 text-white">
            <AlertCircle className="w-5 h-5" />
          </div>
          <div className="text-left">
            <h3 className="text-xs sm:text-sm font-semibold text-slate-900 mb-0.5 font-['Poppins']">{t('importantNoteTitle')}</h3>
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-12 gap-6 lg:gap-8 items-start">
          ...
          <div className="w-full lg:col-span-7 space-y-4 sm:space-y-6">
            ...
            {/* Device Filter Tab */}
            <div className="bg-white border border-slate-200/80 p-1.5 sm:p-2 rounded-2xl shadow-sm flex items-center gap-1.5 sm:gap-2">
              <button
                type="button"
                onClick={() => setSelectedDevice('apple')}
                className={`flex-1 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold rounded-xl transition-all border-none cursor-pointer flex items-center justify-center gap-1.5 sm:gap-2 font-['Poppins'] ${
            ...
            <div className="bg-white border border-slate-200/80 rounded-[24px] sm:rounded-[28px] p-4 sm:p-6 shadow-sm space-y-4">
              ...
              <div className="flex items-center justify-between pb-3 border-b border-slate-100">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-xs sm:text-base font-semibold text-slate-900 font-['Poppins']">{t('installationSteps')}</h2>
                </div>
          ...
          <div className="w-full lg:col-span-5 space-y-4 sm:space-y-6">
            ...
            <div className="bg-white border border-slate-200/80 rounded-[24px] sm:rounded-[28px] p-4 sm:p-6 shadow-sm space-y-3 text-left">
              <div className="flex items-center gap-2.5 pb-2 border-b border-slate-100">
                ...
                <h2 className="text-sm sm:text-base font-semibold text-slate-900 font-['Poppins']">{t('troubleshootingTips')}</h2>
              </div>
